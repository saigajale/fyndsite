...
import {
  HEALTHCARE_PROVIDER_LABEL,
  activeHealthcareProviderName,
  healthcarePlacesProvider,
} from "@/lib/healthcare/healthcare";
  ...
  try {
    ...
    return NextResponse.json({
      ...
      meta: {
        ...
        longitude,
        radiusMetres,
        provider: HEALTHCARE_PROVIDER_LABEL[activeHealthcareProviderName],
        providerEndpoint,
      },
    });
  } catch (error) {
    if (error instanceof HealthcareProviderError) {
      if (error.kind === "timeout") {
        return NextResponse.json(
          { error: "The healthcare data source timed out. Please try again." },
          { status: 504 },
        );
      }
      ...
      if (error.kind === "unconfigured" || error.kind === "not_implemented") {
        return NextResponse.json(
          {
            error: "The selected healthcare data provider is not available right now.",
          },
          { status: 503 },
        );
      }
